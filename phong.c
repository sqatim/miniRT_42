...
/*   Updated: 2020/10/21 14:50:05 by sqatim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_colour ft_diffuse(t_data *type)
{
    ...
    // produit = colour_prd(type->objet->colour, type->light->rgb_light);
    ...
    //bash ntesti dot o diman kaykhruj negatif;
    // printf("|dot ===> %f|\n",dot);
    // printf("|r ====> %f|\t|g ====> %f|\t|b ====> %f|\n", type->objet->colour.r,type->objet->colour.g,type->objet->colour.b);
    // printf("|r ====> %f|\t|g ====> %f|\t|b ====> %f|\n", produit.r,produit.g,produit.b);
    // printf("|r ====> %f|\t|g ====> %f|\t|b ====> %f|\n", diffuse.r,diffuse0.g,diffuse.b);
    return (diffuse);
}
...
t_colour phong(t_data *type, t_colour shadow)
{
    ...
    while (type->light != NULL)
    {
        ...
        type->phong.specular = colour_add(type->phong.specular, ft_specular(type));
    ...
    phong_s = colour_prd(shadow,somme);
     // badalt hna phong_s b somme bash nhayad shading;
